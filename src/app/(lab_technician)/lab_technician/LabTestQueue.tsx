...
const LabTestQueue: React.FC<LabTestQueueProps> = ({ onSelectLabReport, setActiveTab }) => {
  ...
  // Fetch Reports
  const fetchLabReports = useCallback(async (reset = false) => {
    ...
    try {
      ...
      const data: LabReport[] = json.data || json;
  ...
  }, [page, searchTerm, filterStatus, filterDate]);
  ...
  const getPriorityLabel = (createdAt: string) => {
    const hoursAgo = (new Date().getTime() - new Date(createdAt).getTime()) / (1000 * 60 * 60);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 relative">
        ...
        <div className="p-6">
          ...
          {labReports.length === 0 && !initialLoading ? (
                  ...
                  >
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="min-w-0">
                            <h4 className="font-semibold text-gray-900 truncate">
                              {report.student?.name || report?.prescription?.other_name || "Unknown"}
                            </h4>
                            <p className="text-sm text-gray-600 truncate">ID: {report.student?.id_number || "--"}</p>
                      ...
                      <div className="lg:col-span-2">
                        <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">{report.prescription?.doctor_notes || "No notes"}</p>
                          </div>
                      ...
                      <div className="lg:col-span-1 flex flex-col justify-between">
                        <div className="mb-4">
                          <div className="flex items-center space-x-1">
                            <AlertTriangle className={`w-4 h-4 flex-shrink-0 ${getPriorityColor(report.created_at)}`} />
                            <span className={`text-sm font-medium ${getPriorityColor(report.created_at)}`}>
                              {getPriorityLabel(report.created_at)}
                            </span>
                          </div>
                        </div>
